...
export const getLocalWallet = async (localKeyLocalStorageKey: string) => {
  ...
  if (storedJwkString) {
    ...
    const creationResult = await createWalletFromJwk(storedJwk, true)(dummyConnectConfig);
  ...
  const creationResult = await createWalletFromJwk(newJwk, true)(dummyConnectConfig);
  if (creationResult.success) {
    return creationResult.result;
  } else {
